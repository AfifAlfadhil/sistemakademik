...
def preprocess_image_for_ocr(
    ...
    bilateral_sigma_color: int = None,
    bilateral_sigma_space: int = None,
    adaptive_block_size: int = None,
    adaptive_c: int = None,
    deskew_threshold: float = None,
    border_crop_percent: float = None,
) -> Image.Image:
    prep_cfg = config.get("ocr", {}).get("preprocessing", {})
    bilateral_d = bilateral_d or prep_cfg.get("bilateral_d", 9)
    bilateral_sigma_color = bilateral_sigma_color or prep_cfg.get("bilateral_sigma_color", 75)
    bilateral_sigma_space = bilateral_sigma_space or prep_cfg.get("bilateral_sigma_space", 75)
    adaptive_block_size = adaptive_block_size or prep_cfg.get("adaptive_block_size", 15)
    adaptive_c = adaptive_c or prep_cfg.get("adaptive_c", 8)
    deskew_threshold = deskew_threshold or prep_cfg.get("deskew_threshold", 0.5)
    if border_crop_percent is None:
        border_crop_percent = prep_cfg.get("border_crop_percent", 0.02)
    """
